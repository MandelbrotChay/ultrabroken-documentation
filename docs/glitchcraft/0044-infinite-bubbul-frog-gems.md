---
title: "Infinite Bubbul Frog Gems"
---

# Infinite Bubbul Frog Gems

## Overview
Allows you to get infinite bubbul gems

## How To
```
Only works in some caves!
1. Shoot the bubbul frog
2. Ascend during it's death
3. Cancel the ascend
4. Shoot the bubbul frog
5. Repeat
```


## Metadata
## Notes
The glitch may be not exclusive to bubbulfrogs (just a theory)
Needs to be tested more, and see if it can be consistently replicated.

May 21, 2023(?)

## Credit
—

## Date
—
## Source

—

## Examples

- [YouTube](https://youtu.be/c9TDP4qdAUY)

## Resources

—

## Working Versions

- 1.0.0
- 1.1.0
- 1.1.1

## See also

—

## Raw row
```csv
Glitch: Infinite Bubbul Frog Gems, Glitch Overview: Allows you to get infinite bubbul gems, How To: Only works in some caves!
1. Shoot the bubbul frog
2. Ascend during it's death
3. Cancel the ascend
4. Shoot the bubbul frog
5. Repeat, Working Versions: 1.0.0
1.1.0/1.1.1, Date: May 21, 2023, Effects/Extensions: The glitch may be not exclusive to bubbulfrogs (just a theory), Example(s): https://youtu.be/c9TDP4qdAUY, Notes: Needs to be tested more, and see if it can be consistently replicated.

May 21, 2023(?)
```
