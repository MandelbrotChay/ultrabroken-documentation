---
title: "Mineru Text Storage"
---

# Mineru Text Storage

## Overview
Lets you move while text is on screen

## Instructions
```
1. Escape the last battle, to get the real sages to follow Link

2. Either fuse to an item in Hateno Village, try to enter Gerudo Town wearing the wrong gear or activate a Yiga ambush

3. You'll keep moving while the text is on screen, causing various loading issues and allowing for super far camera zoom outs
```

## Notes
Only tested on 1.0.0, probably works on all versions

## Credit
CM30

## Date
July 11, 2024

## Source

- [Discord](https://discord.com/channels/1086729144307564648/1128894970653327401/1261062222306869268)

## Examples

- [Youtube](https://www.youtube.com/watch?v=W0bNi0q7A6Q)

## Resources

—

## Versions

- 1.0.0

## See also

—

## Raw row
```csv
Glitch: Mineru Text Storage, Glitch Overview: Lets you move while text is on screen, How To: 1. Escape the last battle, to get the real sages to follow Link

2. Either fuse to an item in Hateno Village, try to enter Gerudo Town wearing the wrong gear or activate a Yiga ambush

3. You'll keep moving while the text is on screen, causing various loading issues and allowing for super far camera zoom outs, Working Versions: 1.0.0, Credit: CM30, Date: July 11, 2024, Link to Source: Discord, Example(s): Youtube, Notes: Only tested on 1.0.0, probably works on all versions
```
