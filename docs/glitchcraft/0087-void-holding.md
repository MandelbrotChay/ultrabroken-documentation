---
title: "Void Holding"
---

# Void Holding

## Overview
Allows you to hold materials "whenever you want" as long as you are at a low enough Y coordinate

## Instructions
```
Clip below roughly Y -4000 and hold any stack of items
```


## Notes
Can cause a weird hold smuggeling state that usually softlocks the game after a few seconds, however you can still do actions (e.g. warping), as long as you do it before the softlock
Needs more testing

## Credit
Lightos_

## Date
June 10, 2023

## Source

- [Discord](https://discord.com/channels/1086729144307564648/1105598687167664239/1117164294505779244)

## Examples

- [Discord](https://discord.com/channels/1086729144307564648/1105598687167664239/1117479024063684660)

## Resources

—

## Working Versions

- 1.0.0
- 1.1.0
- 1.1.1

## See also

—

## Raw row
```csv
Glitch: Void Holding, Glitch Overview: Allows you to hold materials "whenever you want" as long as you are at a low enough Y coordinate, How To: Clip below roughly Y -4000 and hold any stack of items, Working Versions: 1.0.0
1.1.0/1.1.1, Credit: Lightos_, Date: June 10, 2023, Link to Source: Discord, Effects/Extensions: Can cause a weird hold smuggeling state that usually softlocks the game after a few seconds, however you can still do actions (e.g. warping), as long as you do it before the softlock, Example(s): https://discord.com/channels/1086729144307564648/1105598687167664239/1117479024063684660, Notes: Needs more testing
```
