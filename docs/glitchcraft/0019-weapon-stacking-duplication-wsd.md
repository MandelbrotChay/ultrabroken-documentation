---
title: "Weapon Stacking Duplication (WSD)"
---

# Weapon Stacking Duplication (WSD)

## Overview
allows for a quick dupe of any weapon, bow or shield

## Instructions
```
INEFFICIENT METHOD, SEE NEXT COLUMN
Crouch, pause, and then drop 5 (preferrably 2 handed and fused) weapons on the ground with a wall about 1 link length behind you. Swap to the (equipped) inventory item you wish to dupe. Drop that item, select another, and rapidly pause-unpause. Repeat this as many times as you wish.
Additionally, weapon smuggling can be performed and stored through saves by mashing A through the pause-unpause process during WSD.
```

## Notes
This is just zuggling, but you drop your equipped weapon at the end, that being said, any zuggle method should work (map zuggle is reccomended)

## Credit
Erling附身

## Date
May 16, 2023

## Source

- [Bilibili](http://www.bilibili.com/video/BV1qT411t7Kv/)

## Examples

—

## Resources

—

## Versions

- 1.0.0
- 1.1.0
- 1.1.1

## See also

—

## Raw row
```csv
Glitch: Weapon Stacking Duplication (WSD), Glitch Overview: allows for a quick dupe of any weapon, bow or shield, How To: INEFFICIENT METHOD, SEE NEXT COLUMN
Crouch, pause, and then drop 5 (preferrably 2 handed and fused) weapons on the ground with a wall about 1 link length behind you. Swap to the (equipped) inventory item you wish to dupe. Drop that item, select another, and rapidly pause-unpause. Repeat this as many times as you wish.
Additionally, weapon smuggling can be performed and stored through saves by mashing A through the pause-unpause process during WSD., Working Versions: 1.0.0
1.1.0/1.1.1, Credit: Erling附身, Date: May 16, 2023, Link to Source: Bilibili, Effects/Extensions: This is just zuggling, but you drop your equipped weapon at the end, that being said, any zuggle method should work (map zuggle is reccomended)
```
