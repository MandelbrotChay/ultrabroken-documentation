document.addEventListener('DOMContentLoaded', function () {
  ...
  function updateReserve() {
    ...
    // Allow marquee to activate by not over-reserving space
    const zoomBoost = 0; // no boost - let titles clip naturally
    const boosted = rawAvailable + zoomBoost;
    // Conservative minimum to allow scrolling on longer titles
    const minAvailable = Math.max(200, Math.round(window.innerWidth * 0.35));
