  ...
  document.addEventListener('DOMContentLoaded', ()=>{
    ...
    */
    (function(){
      ...
      function ensureCSS(){
        if (!document.querySelector('link[href$="ai-search.css"]')){
          const link = document.createElement('link');
          link.rel = 'stylesheet';
          link.href = '/assets/stylesheets/ai-search.css';
      ...
      async function askWorker(q){
        ...
        const url = window.AI_WORKER_URL || localStorage.getItem('ai_worker_url') || DEFAULT_WORKER_URL;
        try{
          const res = await fetch(url, { method:'POST', headers: {'Content-Type':'application/json'}, body: JSON.stringify({ query: q, debug: false }) });
          if (!res.ok) throw new Error('worker error '+res.status);
          return await res.json();
        }catch(e){ return { error: String(e) }; }
      }
